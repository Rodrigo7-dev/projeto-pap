<template>
  <div class="min-h-screen bg-slate-50 p-6">
    <div class="max-w-6xl mx-auto">
      <h1 class="text-3xl font-bold text-slate-800 mb-8">Freguesias</h1>
      ...
      <div v-else>
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="freguesia in filteredFreguesias" :key="freguesia.id" 
              ...
              <div class="flex items-start justify-between">
                <div>
                  <h3 class="font-medium text-slate-800">{{ freguesia.freguesia }}</h3>
                  <p class="text-sm text-slate-500 mt-1">ID: {{ freguesia.id }}</p>
                </div>
        ...
        <div class="mt-6 bg-white rounded-lg shadow p-4">
          <div class="text-center text-slate-600">
            Total de freguesias: <span class="font-bold text-slate-800">{{ freguesias.length }}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.carregarFreguesias()
  },

  methods: {
    async carregarFreguesias() {
      this.loading = true
      try {
        const response = await api.get('/freguesias')
        this.freguesias = response.data.data || response.data || []
      } catch (error) {
